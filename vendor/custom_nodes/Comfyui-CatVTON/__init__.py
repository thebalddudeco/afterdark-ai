...
class LoadAutoMasker:
    ...
    def load(self, catvton_path):
        ...
        detectron_root = os.path.join(node_root, "model", "SCHP", "mhp_extension", "detectron2")
        densepose_root = os.path.join(detectron_root, "projects", "DensePose")
        for runtime_path in [densepose_root, detectron_root]:
...
class CatVTON:
    ...
    def generate(
        self, pipe, target_image, refer_image, mask_image, seed, steps, cfg
    ):
